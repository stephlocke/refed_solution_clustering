...
sol_summary_hl %>%
    ...
    select(-ends_with(".x")) %>% 
    mutate(profit = annual_us_dollars_net_financial_benefit / max(annual_us_dollars_gross_financial_benefit,1)) %>%
...
Let's see just cluster 4 sorted by priorities.
```{r cluster_3}
assignments %>%
  filter(k==5, .cluster==4) %>%
  select(solution_name, .cluster) %>%
  inner_join(sol_hl) %>%
