...
sol_finance_stakeholder  %>%
    filter(!stakeholder %in% c("consumer","government")) %>%
    group_by(solution_group,solution_priority_action_area,solution_name) %>%
    summarise_if(is.numeric,sum) %>%
    ungroup()  ->
    sol_finance_hl
...
sol_summary_hl %>%
    ...
    select(-ends_with(".x")) %>% 
    mutate(profit = annual_us_dollars_net_financial_benefit.y / annual_us_dollars_gross_financial_benefit.y) %>%
    select(solution_name, annual_meal_equivalents_diverted, 
            profit, annual_gallons_water_savings_potential,
            jobs_created, annual_mtco2e_reduction_potential,
            annual_us_dollars_gross_financial_benefit.y) ->
    sol_hl
...
```{r cluster_groups}
p1 = ggplot(assignments, aes(x = profit, y = annual_meal_equivalents_diverted)) +
...
assignments %>%
    ...
    select(solution_name, .cluster) %>%
    inner_join(sol_hl) %>%
    arrange(.cluster, desc(annual_us_dollars_gross_financial_benefit.y))
```

Let's see just cluster 3 sorted by priorities.
```{r cluster_3}
assignments %>%
  filter(k==5, .cluster==3) %>%
  select(solution_name, .cluster) %>%
  inner_join(sol_hl) %>%
  arrange(desc(annual_meal_equivalents_diverted), desc(profit),
    desc(annual_us_dollars_gross_financial_benefit.y), desc(jobs_created),
